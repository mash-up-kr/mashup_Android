...
import com.mashup.ui.attendance.platform.PlatformAttendanceState
...
class PlatformAttendanceViewModelTest {
    ...
    fun `when eventNum is 0, notice message is 아직 일정 시작 전이예요`() {
        ...
        // Then
        assertEquals((platformAttendanceViewModel.platformAttendanceState.value as? PlatformAttendanceState.Success)?.notice, "아직 일정 시작 전이예요.")
    }
    ...
    fun `when isEnd is false, notice message is 출석체크가 실시간으로 진행되고 있어요`() {
        ...
        // Then
        assertEquals((platformAttendanceViewModel.platformAttendanceState.value as? PlatformAttendanceState.Success)?.notice, "출석체크가 실시간으로 진행되고 있어요")
    }
    ...
    fun `when isEnd is true & eventNum is 1, notice message is 1부 출석이 완료되었어요`() {
        ...
        // Then
        assertEquals((platformAttendanceViewModel.platformAttendanceState.value as? PlatformAttendanceState.Success)?.notice, "1부 출석이 완료되었어요.")
    }
    ...
    fun `when isEnd is true & eventNum is 2, notice message is 출석체크가 완료되었어요`() {
        ...
        // Then
        assertEquals((platformAttendanceViewModel.platformAttendanceState.value as? PlatformAttendanceState.Success)?.notice, "출석체크가 완료되었어요")
    }
    ...
    fun `when exceptional case, notice message is 서버에서 이상한 일이 발생했어요 ㅜ`() {
        ...
        // Then
        assertEquals((platformAttendanceViewModel.platformAttendanceState.value as? PlatformAttendanceState.Success)?.notice, "서버에서 이상한 일이 발생했어요 ㅜ")
    }
}
